...
public class WeaponMgrUI : MonoBehaviour
{
    ...
    public void UpdateAllPossession()
    {
        for (int i = 0; i < sliders.Length; i++)
        {
            ...
            if ( weapon != null )
            {
                ...
                sliders[i].value = (float)weapon.GetCount() / 5 >= 1 ? 1 : (float)weapon.GetCount() / 5;
                texts[i].text = weapon.GetCount().ToString() + "/5";
            }
        }
    }
    ...
    public void UpdatePossession(int id)
    {
        ...
        if (weapon != null)
        {
            ...
            sliders[id].value = (float)weapon.GetCount() / 5 >= 1 ? 1 : (float)weapon.GetCount() / 5;
            texts[id].text = weapon.GetCount().ToString() + "/5";
        }
    }
